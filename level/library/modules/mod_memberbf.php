...
class mod_member extends module_class{
	...
	function capital(){
	  if($_GET['type']=='list'){
	    ...
	    if($_GET['method']=='money'||$_GET['method']=='regmoney'||$_GET['method']=='shopmoney'||$_GET['method']=='balance'){
	       ...
	       $this->pagetotal = $this->mysql->counts("select id from {$this->pre}earning_log $where ");
	       $this->pageclass($this->pagetotal);
	       $this->record = $this->mysql->getarr("select * from {$this->pre}earning_log $where  order by id desc limit {$this->page->start},{$this->page->size}");
	...
	function goods(){
	    if($_GET['type']=='order'){
	    	...
	    	if($_GET['id']){
			    ...
			    if($this->member['usergroup']['buy_way']){
			    	$paytype[0] = array("type"=>"财付通","payno"=>config::get("tenpay"),"payname"=>config::get("tenpayname"),"payadress"=>"",);
			    	$paytype[1] = array("type"=>"支付宝","payno"=>config::get("alipay"),"payname"=>config::get("alipayname"),"payadress"=>"",);
			    	$paytype[2] = array("type"=>"支付宝","payno"=>config::get("v_mid"),"payname"=>config::get("chinabankname"),"payadress"=>"chinabankaddress",);
			    }
			    if(submit()){
	    		    $arr['huiyuan_img'] = implode(",",$_POST['thumb_list']);
	    		    // print_r($arr['huiyuan_img']);exit;
	    		    if($this->member['groupuser']['buy_way']){
	    		    	$arr['checked'] = 10;
	    		    }else{
	    		    	$arr['checked'] = 9;
	    		    }
			    	...
			    	$tmppaytype = $this->mysql->select_one("select * from {$this->pre}atmbank  where uid='{$this->member['refereeid']}'");
			    	$paytype[0] = array("type"=>$tmppaytype['bankadd'],"payno"=>$tmppaytype['bankcard'],"payname"=>$tmppaytype['truename'],"payadress"=>$tmppaytype['bankname']);
			    }
			    // echo '<pre>';
		...
		if($_GET['type']=='sendorder'){
	    	if($_GET['id']){
			    if(submit()){
	    		    $arr['agent_img'] = implode(",",$_POST['agent_img']);
	    		    $arr['checked'] = 10;
	    		    $this->mysql->update("{$this->pre}order",$arr,"id='{$_GET['id']}'"); 
	    	    }
		  	    $this->order = $this->mysql->select_one("select * from {$this->pre}order where id='{$_GET['id']}' order by id desc limit 1");
	...
	function store(){
	    ...
	    if($_GET['type']=='detail'){
			...
			$this->delivery = $this->mysql->getarr("select * from {$this->pre}delivery where uid='{$this->member['uid']}'");
		    $this->pagetotal = $this->mysql->counts("select goods_id from {$this->pre}goods $where");
		    $record = $this->sqlgoods("select * from {$this->pre}goods where goods_id=9 order by goods_id desc {$this->page->limit}");
		    foreach ($record as $key => $value) {
		    	$tmp_group = $this->mysql->select_one("select * from {$this->pre}shop_group where goodid={$value['goods_id']} and groupid={$this->member['usergroup']['groupid']}");
		    	$record[$key]['agent_price'] = $value['agent_price'] ;
