...
class mod_member extends module_class{
	    	...
	    	if($_GET['id']){
			    if(submit()){
			    	if($this->member['usergroup']['buy_way']){
			    		$arr['agent_img'] = implode(",",$_POST['thumb_list']);
		    		    $arr['cuid'] = $this->member['uid'];
		    		  	$arr['checked'] = 10;
		    		    $picarr['pic'] = implode(",",$_POST['thumb_list']);
		    		    $picarr['uid'] = $this->member['uid'];
		    		    $picarr['oid'] = $_GET['id'];
		    		    $picarr['pid'] = $this->member['refereeid'];
		    		    $picarr['addtime'] = time();
		    		    $this->mysql->insert("{$this->pre}paypic",$picarr);
			    	}else{
			    		$arr['checked'] = 1;
			    		$arr['send_type'] = 'user';
			    	}
		        ...
		        if($_GET['method']){
			  	...
			  	if($_GET['orderid']) $where .= " and orderid like '{$_GET['orderid']}%'";
			        ...
			        while($rs=$this->mysql->assoc($query)){
						...
						$rs['addtime'] = formattime($rs['addtime']);
						$pic = $this->mysql->select_one("select * from {$this->pre}paypic where oid='{$rs['oid']}' and uid='{$this->member['uid']}'");
						$rs['agent_img'] = $pic['pic'];
				        $this->order[] = $rs;
			        }
		      	}
		}
	...
	function capital(){
	  if($_GET['type']=='list'){
		...
		if($_GET['method']=='main'){
           ...
           $todaytime = untime(formattime(time(),'Y-m-d'));
           $yestodaytime = untime(formattime(time()-24*3600,'Y-m-d')); 
           $this->todaymoney = $this->getrecords("where jid='{$this->member['uid']}' and addtime='{$todaytime}'");
	       $this->yestodaymoney = $this->getrecords("where jid='{$this->member['uid']}' and addtime='{$yestodaytime}'");
	       $this->allmoney = $this->getrecords("where jid='{$this->member['uid']}'");
	...
	function jfgoods(){
	    ...
	    if($_GET['type']=='list'){
			...
			//代理产品页面加载
		    $record = $this->mysql->select_one("select * from {$this->pre}jfgoods");
			    ...
			    if(submit()){
			    	$this->order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}' order by id desc limit 1");
	    		   	...
	    		   	if($this->order['checked']==0){
	    		   		...
	    		   		$arr['refereeid'] = $this->member['refereeid'];
	    		   		$this->mysql->update("{$this->pre}jforder",$arr,"id='{$_GET['id']}'"); 
	   			  		$json['message'] = '凭证上传成功！';
