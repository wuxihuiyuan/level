<?php
class mobile_class extends user_class{
	...
	public function check() {
	  ...
	  if(($uid==''||$shell=='')&&$this->verifyLgoin){
        $this->logout('?mod=mobile&act=login&url='.base64_encode(rewrite::response(geturl())),'请登陆后进行操作');
		return false;
	  }else{
	...
	public function login($username,$password,$seccode) {
	  if($username=='') $this->logout('?mod=mobile&act=login&url='.$_GET['url']);
	  if($password=='') $this->logout('?mod=mobile&act=login&url='.$_GET['url']);
	  if(config::get("mustcode")&&$seccode=='') $this->logout('?mod=mobile&act=login&url='.$_GET['url']);
	  if(config::get("mustcode")&&$seccode!=$_SESSION['seccode']) $this->logout('?mod=mobile&act=login&url='.$_GET['url']); 
	  if(isMobile($username)){
	    if(!is_array($row = $this->sql($username,'userphone',"mcheck='1'"))) $this->logout('?mod=mobile&act=login&url='.$_GET['url']);		
	  }elseif(isEmail($username)){
	    if(!is_array($row = $this->sql($username,'email'))) $this->logout('?mod=mobile&act=login&url='.$_GET['url']);	
	  }else{
	  	$row = $this->sql($username,'username');
	  
        if(!is_array($row)) $this->logout('?mod=mobile&act=login&url='.$_GET['url']);	 
	  }

	  if(!$row['canlogin']) $this->logout('?mod=mobile&act=login&url='.$_GET['url']);
	  if($this->password($password,$row[salt]) == $row[password]){//密码是否正确
		...
		location($location);
  	    ...
  	    $this->logout('?mod=mobile&act=login&url='.$_GET['url']);		  
	  }  			
	}
	...
	function purview(){
	  $mobile_purviews = $this->module->mobile['purviews'];
	  $purviews = explode(',',$mobile_purviews);
	...
	function gt_purview($act){
	  $purviews = $this->module->mobile['purviews'];
	...
	public function relogin($password){  
	 ...
	 if($this->password($password,$this->module->mobile['salt']) == $this->module->mobile['repass']){
	   $this->keeprepass($this->module->mobile['uid']);
	 }else{
	...
	function checkpassword($password,$pay=''){
		 $checkpassword = $pay=='' ? $this->module->mobile['password'] : $this->module->mobile['repass'];
		 return $this->password($password,$this->module->mobile['salt'])==$checkpassword;
	}		
