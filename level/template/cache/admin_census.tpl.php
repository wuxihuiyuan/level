...
<div class="headbar clearfix">
  <ul class="tab">
    <li <? if($_GET['type']=='') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre()); ?>">综合统计</a></li>
    <li <? if($_GET['type']=='1') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre().'&type=1'); ?>">现金币记录</a></li>
    <div class="tabright">
...
<div class="headbar clearfix">
  <ul class="tab">
    <li <? if($_GET['type']=='') { ?>class="selected"<? } ?>><a href="<?=preg_replace('#&type=(\d+)#','',geturl()); ?>">曲线走势图</a></li>
    <li <? if($_GET['type']=='1') { ?>class="selected"<? } ?>><a href="<?=preg_replace('#&type=(\d+)#','',geturl()).'&type=1'; ?>" >柱状走势图</a></li>
    <div class="tabright">
      <form method="get" action="">
        <input type="hidden" name="mod" id="mod" value="<?=$_GET['mod']?>" />
        <input type="hidden" name="act" id="act" value="<?=$_GET['act']?>" />
...
<script language="javascript"> 
$(function(){
   chart('container',"<?=$_GET['type']=='1' ? 'column' : 'line'; ?>",[<?=$this->categories?>],[{name:'拨出奖金',data:[<?=$this->outmoney?>]},{name:'业绩收入',data:[<?=$this->buymoney?>]}]);	   
});
</script>
<? } } if($_GET['get']=='atmlog') { if($_GET['re']=='list') { ?>
<div class="headbar clearfix">
  <ul class="tab">
    <li <? if($_GET['type']=='') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre()); ?>">全部记录</a></li>
    <li <? if($_GET['type']=='1') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre().'&type=1'); ?>">暂未汇款</a></li>
    <li <? if($_GET['type']=='2') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre().'&type=2'); ?>">已经汇款</a></li>
    <div class="tabright">
      <form method="get" action="">
        <input type="hidden" name="mod" id="mod" value="<?=$_GET['mod']?>" />
        <input type="hidden" name="act" id="act" value="<?=$_GET['act']?>" />
...
<form method="post" name="atmpost" id="atmpost" action="" onsubmit="return atmsubmit();">
  <table border="0" cellspacing="2" cellpadding="4" class="list" name="table" id="table" width="100%">
    ...
    <? if(is_array($this->atmlog)) { foreach($this->atmlog as $value) { ?>    <? $user = $this->mysql->select_one("select * from ".$this->pre."user where uid=".$value['uid']); ?>    <tbody>
      <tr class="trhover">
...
<div class="headbar clearfix">
  <ul class="tab">
    <li <? if($_GET['type']=='') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre()); ?>">全部记录</a></li>
    <li <? if($_GET['type']=='1') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre().'&type=1'); ?>">暂未支付</a></li>
    <li <? if($_GET['type']=='2') { ?>class="selected"<? } ?>><a href="<?=Purl(adminpre().'&type=2'); ?>">支付成功</a></li>
    <div class="tabright">
      <form method="get" action="">
        <input type="hidden" name="mod" id="mod" value="<?=$_GET['mod']?>" />
        <input type="hidden" name="act" id="act" value="<?=$_GET['act']?>" />
...
<table border="0" cellspacing="2" cellpadding="4" class="list" name="table" id="table" width="100%">
  ...
  <? if(is_array($this->paylog)) { foreach($this->paylog as $value) { ?>  <tbody>
    <tr class="trhover">
      <td align="center"><?=$value['id']?></td>
      ...
      <td align="center" value="<?=$value['checked']?>" id="paylog_<?=$value['id']?>"> 
        <?=paycheck($value['checked'],$value['orderid'],$value['id']); ?>        <? if($value['checked']=='0') { ?>
...
<script language="javascript"> 
$(function() {
   chart('container',"<?=$_GET['type']=='1' ? 'column' : 'line'; ?>",[<?=$this->categories?>],[{name:'人工充值',data:[<?=$this->_paymoney?>]},{name:'在线充值',data:[<?=$this->paymoney?>]},{name:'合计充值',data: [<?=$this->allpaymoney?>]}]);    
});
</script>
