...
function _input(obj,id,min,bonus){
  ...
  var agent_price = $('.info_'+id).data('value');
  var bonus = $('.info_'+id).data('bonus');
  getprice(id,number,bonus,agent_price);
}
function getprice(sid,number,bonus,agent_price){
  var price = parseInt(number)*parseInt(agent_price);
  $('#price').html(price);
  var jfprice = parseInt(number)*parseInt(agent_price);
  $('#jfprice').html(jfprice);
  $('input[name="jfprice"]').val(jfprice);
}
function get_price(){
  var checked = $('.table').find('input[type=checkbox]:checked');
  var price=0;
