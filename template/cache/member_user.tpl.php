...
<? } else { ?>
<span class="tips2 emali-no">邮箱未验证。</span>
<a class="abut02" href="<?=Purl(" ?mod=member&act=user&type=authemail "); ?>"><span>去验证&nbsp;&gt;&gt;</span></a>
<? } ?>
</div>
</div>
<div class="opencard_h">
...
<? } else { ?>
<span class="tips2 phone-no">手机未绑定。</span>
<a class="abut02" href="<?=Purl(" ?mod=member&act=user&type=authphone "); ?>"><span>去绑定&nbsp;&gt;&gt;</span></a>
<? } ?>
</div>
</div>
<!-- 
...
<? } if($_GET['type']=='password') { if($_GET['method']=='paypasswd') { ?>
<script language="javascript">
var paypasswdtip = "支付";
</script>
<? } ?>
<div class="track_title">
<a class="<?=$_GET['method']=='' ? 'menushow' : 'menu'; ?>" href="<?=Purl(" ?mod=member&act=user&type=password "); ?>">修改登陆密码</a>
<a class="<?=$_GET['method']=='paypasswd' ? 'menushow' : 'menu'; ?>" href="<?=Purl(" ?mod=member&act=user&type=password&method=paypasswd "); ?>">修改支付密码</a>
</div>
<div class="order-prompt order-promptbommton">请务必确认登录密码与支付密码完全不同！密码相同会导致支付密码失效，使您的帐户资金无法得到安全保障。</div>
<? if($_GET['method']=='') { ?>
...
<span class="text">请尽快登录您的邮箱点击激活链接完成验证。</span>
<a class="abut03" href="<?=$this->emailurl?>" target="_blank"><em>去邮箱</em></a>
<div class="verify-text"> <b>收不到邮件？</b>
<ul>
<li>有可能被误判为垃圾邮件了，请到垃圾邮件文件夹找找。</li>
<li>拨打客服电话，让客服来帮您解决：<?=config::get('sitephone')?>。</li>
<li>或者选择
<a class="abut02" href="<?=Purl(" ?mod=member&act=user&type=authemail "); ?>"><span>重发验证邮件</span></a>
</li>
</ul>
</div>
</div>
...
<th>是否默认</th>
<th>操作</th>
</tr><? if(is_array($this->record)) { foreach($this->record as $value) { ?><tr class="mybg" id="remove_<?=$value['id']?>">
<td width="150"><?=$value['name']?></td>
<td><?=$value['address']?></td>
<td><?=$value['mobile']?></td>
<td><?=$value['is_default']?></td>
<td>
<a href="javascript:listTable.addressRemove('<?=$value['id']?>','确定要删除该地址吗');"style="color: #f00; margin-right: 10px;">删除</a>
<a href="<?=Purl(" ?mod=member&act=user&type=address&method=add&id=".$value['id']); ?>" style="color: #1c961c;">编辑</a>
</td>
</tr><? } } ?></table>
...
<div class="opencard_input_box">
是<input name="is_default" type="radio" value="1" <? if($this->detail[is_default] == 1) { ?>checked<? } ?> /> 否
<input name="is_default" type="radio" value="0" <? if($this->detail[is_default] == 0) { ?>checked<? } ?> />
</div>
</div>
<div class="opencard_button_box"><?=config::form('opcardbutton','确 定','submit');?></div>
</div>
...
<td>
<a href="javascript:listTable.addressRemove('<?=$value['id']?>','确定要删除该地址吗');" style="color: #f00; margin-right: 10px;">删除</a>
<a href="<?=Purl(" ?mod=member&act=user&type=bankinfo&method=add&id=".$value['id']); ?>" style="color: #1c961c;">编辑</a>
</td>
</tr><? } } ?></table>
<? if(!is_array($this->record)) { ?>
<div class="no_info"><span class="no_info_ico"></span>暂无任何记录</div>
...
<div class="opencard_h">
<div class="opencard_text"><span class="text_x_12px">*</span> 开户姓名</div>
<div class="opencard_input_box">
<input name="truename" class="myinput" type="text" value="<?=$this->detail['truename']?>" />
</div>
</div>
<div class="opencard_h">
