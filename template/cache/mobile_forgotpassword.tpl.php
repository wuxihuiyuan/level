...
<title><?=config::get('sitename')?> - 密码找回</title>
<meta name="keywords" content="<?=$this->keywords?>" />
<meta name="description" content="<?=$this->description?>" />
<? include template('../common','default/mobile'); ?>
<link rel="stylesheet" href="<?=$this->tempdir?>css/login.css" type="text/css" />
<script type="text/javascript" src="<?=$this->tempdir?>js/forgotpassword.js" ></script>
</head>
<body>
<div class="loginheader"></div>
<div class="logincontent">
  <div class="login-title"> 
    <a href="<?=Purl('mobile_forgotpassword')?>" <? if($_GET['do']=='') { ?>class="current"<? } ?>>邮件找回</a> 
    <a href="<?=Purl("?mod=mobile&act=forgotpassword&do=mobile"); ?>" <? if($_GET['do']=='mobile') { ?>class="current"<? } ?>>短信找回</a> 
  </div>
  <? if($_GET['do']=='mobile') { ?>
  <form name="postform" method="post" id="postform" enctype="multipart/form-data" onsubmit="return checkphoneform()">
      <table class="lotab">
        <tr>
          ...
          <td><input id="userphone" name="userphone" class="myinput repass" type="text" value="{mobile userphone}" placeholder="请输入手机号码" />
        ...
        <tr>
          ...
          <td><input name="checkcode" id="checkcode" class="myinput" type="text" value="" onblur="checkcheckcode()" placeholder="手机接收到的验证码" />
          ...
          <tr>
            ...
            <td><input name="password" id="password" class="myinput" type="password" onblur="checkpassword()" onfocus="getpassword();" onkeyup="pwStrength(this.value)" />
              ...
              <div class="grade-pwd"> <span>密码强度：</span>
                ...
                <span class="grade-text"></span> </div></td>
          </tr>
          <tr>
        ...
        <tr>
          <th></th>
          <td><?=config::form('submit','重置密码','submit','','class=\'osLoginIptBtn\'');?> <a href="<?=Purl('mobile_login')?>">返回登录</a></td>
        </tr>
      </table>
  </form>
  <? } else { ?>
  ...
  <div class="forgot-emailok" style="padding-top:20px;">
    ...
    <span class="text">验证邮件24小时内有效，请尽快登录您的邮箱点击激活链接完成验证。</span> <a class="osLoginIptBtn" style=" margin-left:170px;" href="<?=$this->emailurl?>" target="_blank">去 邮 箱</a>
    ...
    <div class="verify-text" style="width:395px;"> <b>收不到邮件？</b>
      <ul>
        ...
        <li>或者选择<a class="abut02" href="<?=Purl('mobile_forgotpassword')?>"><span>重发验证邮件</span></a></li>
      </ul>
    </div>
  </div>
  <? } else { ?>
  <form name="postform" method="post" id="postform" enctype="multipart/form-data" onsubmit="return checkform()">
    <table class="lotab">
      ...
      <tr>
        ...
        <td><input id="seccode" class="myinput yzm" type="text" name="seccode" value="" placeholder="请输入验证码" maxlength="4" onblur="checkseccode();"/>
          <?=config::seccode("class=\"yzmimg\"","width=\"80\" height=\"31\""); ?></td>
      </tr>
      <tr>
        <th></th>
        <td><?=config::form('submit','找回密码','submit','','class=\'osLoginIptBtn\'');?> <a href="<?=Purl('mobile_login')?>">返回登录</a></td>
      </tr>
    </table>
  </form>
