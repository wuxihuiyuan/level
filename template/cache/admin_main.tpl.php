<? if (!defined('ROOT')) exit('Can\'t Access !'); include template('admin_header','admin'); if($_GET['get']=='system') { ?>
<div id="index_pool">
...
['分红奖励', { self allmoney[floormoney] }],
['直荐奖励', { self allmoney[refereemoney] }],
['股东奖励', { self allmoney[money] }],
['报单奖励', { self allmoney[regmoney] }]
]);
});
</script>
<? } if($_GET['get']=='config') { ?>
<form method="post" action="" name="configform" id="configform" enctype="multipart/form-data" onsubmit="return configcheckForm()">
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="table" class="tad"><? $value = $this->configs[$_GET['re']]['arr']; if(is_array($value)) { foreach($value as $input) { ?><tr>
...
<td align="left" style="border:0;"><input type="text" name="filesize" id="filesize" value="100" /> KB
<font color="#666666">(1 MB = 1024 KB)</font>
</td>
...
</tr><? if(is_array($this->database)) { foreach($this->database as $value) { ?><tr>
...
<input type="file" name="updatefile" id="updatefile" /> <?=config::stopoutenable(); ?> &nbsp;&nbsp;
...
<td class="right _right">
<a href="javascript:listTable.unback('<?=$this->managershell?>','<?=$value['filename']?>');">恢复数据</a>
<a href="<?=Purl(" ?mod=admin&act=main&get=database&re=unback&mypath=".$value['filename']." &down=1 "); ?>" target="_blank">打包并下载数据</a>
<a href="javascript:listTable.remove('<?=$value['filename']?>','确定要删除该备份数据吗');">删除备份</a>
</td>
</tr>
</tbody><? } } ?></table>
...
<a href="<?=Purl(adminpre()); ?>">全部信件</a>
</li>
<li <? if($_GET[ 'type']=='1' ) { ?>class="selected" <? } ?>>
<a href="<?=Purl(adminpre().'&type=1'); ?>">收到的信件</a>
</li>
<li <? if($_GET[ 'type']=='2' ) { ?>class="selected" <? } ?>>
<a href="<?=Purl(adminpre().'&type=2'); ?>">发出的信件</a>
</li>
<div class="tabright">
...
<input type="hidden" name="type" id="type" value="<?=$_GET['type']?>" /> 用户名：
<input type="text" name="username" id="username" value="<?=$_GET['username']?>" class='skey' style='width:100px;' /> 时间段：<?=config::form('time',$this->t['str'],'datas');?>
...
</thead><? if(is_array($this->message)) { foreach($this->message as $value) { ?><tbody>
...
<a href="javascript:listTable.message(<?=$value['id']?>,'1');"><?=$value['title']?></a>
</td>
...
<td align="center">
<a href="javascript:listTable.remove('<?=$value['id']?>','确定要删除该信件吗');">彻底删除</a>
</td>
</tr>
</tbody><? } } ?></table>
