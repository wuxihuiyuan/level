...
<!-- 代理产品 -->
<? if($_GET['type']=='list') { if(!$this->member['is_admin']) { ?>
...
<td class="red">&yen; <?=$this->record['mk_price']?>/件</td>
<td class="red">&yen; <?=$this->record['agent_price']?>/件</td>
<td><?=$this->record['unit_rate']?>件/箱</span>
</td>
...
<a href="javascript:_number(1,<?=$this->record['goods_id']?>,<?=$this->record['minimum']?>);"><span class="icon-common icon-common-add"></span></a>箱
</div>
</td>
...
<input name="zhprice" value="" type="hidden" />
<p class="total-price">折后金额：<span><strong id='dzhprice'><?=$this->record['re_money']?></strong>元</span></p>
</div>
</div>
...
<!-- 购买订单结束 -->
<? if($_GET['type']=='order') { if(!$this->member['is_admin']) { ?>
<!-- 详情页 -->
<? if($_GET['id']) { ?>
...
<tbody><? $shop = $this->getgoods(1,"goods_id='".$this->order['goods_id']."'"); ?><tr>
<td class="cell-order-goods">
<div class="order-goods-info" style="width: 30%;"><?=$this->order['good_name']?></div>
<div class="order-goods-price"style="width: 14%;"><?=$shop['agent_price']?>元</div>
<div class="order-goods-amount"style="width: 14%;">x<?=$shop['unit_rate']?>件/箱</div>
<div class="order-goods-amount"style="width: 14%;">x <?=$this->order['num']?>/箱</div>
<? if($this->member['usergroup']['buy_way']) { ?>
<div class="order-goods-amount"style="width: 14%">x <?=$this->order['rate']?>%</div>
<div class="order-goods-amount"style="width: 14%;">- <?=$this->order['bonus']?></div>
<? } ?>
</td>
<td rowspan="<?=count($this->order['goods']); ?>">
<? if($this->member['usergroup']['buy_way']) { ?> <?=$this->order['re_money']?>元 <? } else { ?> <?=$this->order['yi_money']?>元 <? } ?>
</td>
</tr>
</tbody>
</table>
...
<th>账户</th>
<th>地址</th>
</tr><? if(is_array($this->paytype)) { foreach($this->paytype as $val) { ?><tr>
<td><?=$val['type']?></td>
<td><?=$val['payno']?></td>
<td><?=$val['payname']?></td>
<td><?=$val['bankadd']?></td>
...
<div id="uploadPic">
<ul id="uploadlist" style=" height:90px; overflow:hidden;"><? if(is_array($this->add['goods_thumb'])) { foreach($this->add['goods_thumb'] as $k=>$v) { $cover = $k==0 ? "凭证" : "&nbsp;"; ?><li>
<div><img src="<?=$v?>" /></div>
...
function pageInit() {
var uploadurl = '<?=Purl('tools_upload')?>',
ext = '<?=$this->ftype?>',
size = '<?=$this->fsize?>',
count = '<?=$this->fcount?>',
useget = 0,
...
<th>订购时间</th>
<th>目前状态</th>
</tr><? if(is_array($this->order)) { foreach($this->order as $value) { ?><tbody id="remove_<?=$value['id']?>">
<tr class="mybg">
<td width="12%"><?=$value['orderid']?></td>
<td class="order-delivery-table" style="width:40%"><? $shop = $this->getgoods(1,"goods_id='".$value['goods_id']."'"); ?><div class="order-goods-info" style="width:30%;"><?=$shop['goods_name']?></div>
<div class="order-goods-price" style="width:17.5%;"><?=$shop['agent_price']?>元</div>
<div class="order-goods-amount" style="width:17.5%;">x <?=$shop['unit_rate']?>个/箱</div>
<div class="order-goods-amount" style="width:17.5%;">x <?=$value['num']?>箱</div>
<div class="order-goods-amount" style="width:17.5%;">x <?=$value['rate']?>%</div>
<div style="clear:both;"></div>
</td>
<td width="10%" class="red">&yen;<?=$value['money']?></td>
<td width="10%" class="red">&yen;<?=$value['bonus']?></td>
<td width="10%" class="red">&yen;<?=$value['re_money']?></td>
<? if($value['agent_img']) { ?>
<td class="red">
<a href="<?=$value['agent_img']?>" target="_blank"><img style="width: 30px;height: 30px;" src="<?=$value['agent_img']?>" /></a>
</td>
<? } elseif($value['huiyuan_img']) { ?>
<td class="red">
<a href="<?=$value['huiyuan_img']?>" target="_blank"><img style="width: 30px;height: 30px;" src="<?=$value['huiyuan_img']?>" /</a></td>
<? } else { ?>
<td class="red"> 未付款</td>
<? } ?>
<td width="10%"><?=$value['addtime']?></td>
<td width="12%" style="line-height:24px;padding:10px 0;">
<span id="check_user_<?=$value['id']?>" price="<?=$value['price']?>"><?=order_check_user($value); ?></span><br /> <? if(!$value['checked']) { ?>
<a href="<?=Purl(" ?mod=member&act=goods&type=order&id=".$value['id']); ?>" style="background: #4A8DFC;color: #fff;padding: 3px 10px;border-radius: 3px;display: block;margin: 0 auto;width: 48px;position: relative;top: 8px;">确认支付</a>&nbsp;<br />
<a style="background: none;display: block;width: 48px;color: #f00;margin: 0 auto;padding: 2px 9px;border: 1px solid #f00;border-radius: 3px;position: relative;top: -5px;"href="javascript:cancelorder('<?=$value['id']?>');">取消订单</a>
<? } else { ?>
<a href="<?=Purl(" ?mod=member&act=goods&type=order&id=".$value['id']); ?>" style="padding: 2px 9px;width: 48px;margin: 0 auto;display: block;border: 1px solid #11bd11;color: #11bd11;border-radius: 3px;position: relative;top: 10px;">查看详情</a>&nbsp;<br /> <? } if($value['checked']=='5') { ?>
...
<!-- 一级订单 -->
<? if($_GET['type']=='sendorder') { if(!$_GET['id']) { ?>
<div class="track_title">
...
<td width="10%"><?=$value['orderid']?></td>
<td width="10%"><?=$value['username']?></td>
<td class="order-delivery-table" style="width:30%"><? $shop = $this->getgoods(1,"goods_id='".$value['goods_id']."'"); ?><div class="order-goods-info" style="width: 30%;"><?=$shop['goods_name']?></div>
<div class="order-goods-price" style="width: 17.5%;"><?=$shop['agent_price']?>元</div>
<div class="order-goods-amount" style="width: 17.5%;">x <?=$shop['unit_rate']?>个/箱</div>
<div class="order-goods-amount" style="width: 17.5%;">x <?=$value['num']?>箱</div>
<? if($this->member['usergroup']['buy_way']) { ?>
...
<td width="10%" class="red">&yen; <?=$value['price']?>元
</td>
<td width="10%" class="red">&yen; 
<? if($this->member['usergroup']['buy_way']) { ?> <?=$value['money']?>元 <? } else { ?> <?=$value['yi_money']?>元 <? } ?>
</td>
<td width="10%" class="red">&yen; 
<? if($this->member['usergroup']['buy_way']) { ?> <?=$value['re_money']?>元 <? } else { ?> <?=$value['yi_money']?>元 <? } ?>
</td>

<td width="10%"><?=$value['addtime']?></td>
<td width="10%" style="line-height:24px;padding:10px 0;">
<span id="check_user_<?=$value['id']?>" price="<?=$value['price']?>"><?=sendorder_check_user($value); ?></span><br /> 
<? if($value['checked'] == 9) { ?>
</br>
<? if($this->member['usergroup']['sort']>1) { ?>
<a href="<?=Purl(" ?mod=member&act=goods&type=sendorder&id=".$value['id']); ?>" style="background: #4A8DFC;color: #fff;padding: 3px 10px;border-radius: 3px;display: block;margin: 0 auto;width: 48px;position: relative;top: -10px;">订单确认</a>
<? } else { ?>
<input class="order-but order-but2 confirmorder" type="button" orderid="<?=$value['id']?>" value="确认支付" />
<a href="<?=Purl(" ?mod=member&act=goods&type=sendorder&id=".$value['id']); ?>" style="padding: 2px 9px;width: 48px;margin: 0 auto;display: block;border: 1px solid #11bd11;color: #11bd11;border-radius: 3px;position: relative;top: 10px;">查看详情</a>&nbsp;
<? } } else { ?>
<a href="<?=Purl(" ?mod=member&act=goods&type=sendorder&id=".$value['id']); ?>" style="padding: 2px 9px;width: 48px;margin: 0 auto;display: block;border: 1px solid #11bd11;color: #11bd11;border-radius: 3px;position: relative;top: 10px;">查看详情</a>&nbsp; <? } if($value['checked']=='1' && $value['send_type']=='user') { ?>
<br><input class="order-but order-but2 ordersend" type="button" orderid="<?=$value['id']?>" value="发货" />
<? } if($value['num']>$this->member['store'] && $this->mumber['usergroup']['sort']>1) { ?> 库存不足，系统不可发货。 <? } ?>
</tr>
</tbody><? } } ?></table>
<? if(!is_array($this->order)) { ?>
<div class="no_info"><span class="no_info_ico"></span>暂无任何记录</div>
...
<div class="order-text-section-l">
<? if($this->order['huiyuan_img']) { ?>
<div>
<h4 style="display: block;width: 84px;margin-bottom: 10px;">下级凭证：</h4>
<img src="<?=$this->order['huiyuan_img']?>" style="width:355px;" />
</div>
<? } if($this->order['agent_img']) { ?>
<div>
<h4 style="display: block;width: 84px;margin-bottom: 10px;">当前凭证：</h4>
<img src="<?=$this->order['agent_img']?>" style="width:355px;" />
</div>
<? } ?> 
...
<th style="background:none;border:0; text-align:center;display: block;background: #997F41;color: #fff;width: 104px;" align="right" ;>上传凭证：</th>
<td style="background:none; border:0;display: block;position: absolute;top: 185px;left: 5px"><input class="middle" name="" type="hidden" />
<div class="upload_btn"><span id=""></span></div>
<label><span id="upload_tip">图片不超过2MB</span></label></td>
</tr>
</table>
<div id="uploadPic">
<ul id="uploadlist" style=" height:90px; overflow:hidden;"><? if(is_array($this->add['goods_thumb'])) { foreach($this->add['goods_thumb'] as $k=>$v) { $cover = $k==0 ? "凭证" : "&nbsp;"; ?><li>
<div><img src="<?=$v?>" /></div>
<a class="previous" href="javascript:void(0);"></a><span class="front-cover"><?=$cover?></span>
...
<th>用户名</th>
<th>下单数量</th>
</tr><? if(is_array($this->saleslog)) { foreach($this->saleslog as $k => $value) { ?><tbody id="remove_<?=$value['id']?>">
<tr class="mybg">
<td width="12%"><?=$value['id']?></td>
<td width="12%"><?=$value['orderid']?></td>
<td width="10%" class="red"><?=$value['username']?></td>
<td width="10%" class="red"><?=$value['num']?></td>
</tr>
</tbody><? } } ?></table>
<? if(!is_array($this->saleslog)) { ?>
<div class="no_info"><span class="no_info_ico"></span>暂无任何记录</div>
<? } if($this->newpage) { ?>
<div class="pages"><?=$this->newpage?></div>
<? } ?>
