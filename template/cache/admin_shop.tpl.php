...
</thead><? if(is_array($this->goodslist)) { foreach($this->goodslist as $value) { ?><tbody id="remove_<?=$value['goods_id']?>">
<tr class="trhover">
<td align="center"><span class="to_left">+</span><?=$value['goods_id']?></td>
...
<div id="drop_down"><? if(is_array($this->group)) { foreach($this->group as $key=>$value) { ?><ul>
<li style="width: 10%;">一级分销商</li>
<li>起购量：<?=$value['minimum']?>箱</li>
<li>招商奖：<?=$value['rebate']?> 元/箱</li>
<li>提成奖：<?=$value['bonus']?> 元/箱</li>
<li>分红奖：<?=$value['share_money']?> 元/箱</li>
<li>进阶奖：<?=$value['up_bonus']?> 元/箱</li>
</ul><? } } ?></div>
...
<li id="li_1" class="selected">
<a href="javascript:void(0)" hidefocus="true" onclick="select_tab('1')">产品信息</a>
</li>
<li id="li_2">
<a href="javascript:void(0)" hidefocus="true" onclick="select_tab('2')">详细介绍</a>
</li>
<li id="li_3">
<a href="javascript:void(0)" hidefocus="true" onclick="select_tab('3')">产品相册</a>
</li>
<li id="li_4">
<a href="javascript:void(0)" hidefocus="true" onclick="select_tab('4')">代理价设置</a>
</li>
<li id="li_5">
<a href="javascript:void(0)" hidefocus="true" onclick="select_tab('5')">零售设置</a>
</li>
...
<div style="padding:10px;"><?=config::form('goods_desc',$this->add['goods_desc'],'editor','0','
<1@1>style=\'width:580px;height:340px;\'','goods');?></div>
</div>
...
<div id="uploadPic">
<ul id="uploadlist" style=" height:90px; overflow:hidden;"><? if(is_array($this->add['goods_thumb'])) { foreach($this->add['goods_thumb'] as $k=>$v) { $cover = $k==0 ? "封面1" : "&nbsp;"; ?><li>
...
<table border="0" cellspacing="2" cellpadding="4" class="list" name="table" id="table" width="100%">
<tbody><? if(is_array($this->group)) { foreach($this->group as $key=>$value) { ?><tr onmouseover="this.bgColor='#F5F5F5';" onmouseout="this.bgColor='ffffff';" bgcolor="#ffffff">
...
<td>晋级奖： <input type="text" name="group[<?=$value['groupid']?>][up_bonus]" value="<?=$value['up_bonus']?>" />元/箱</td>
</tr><? } } ?></tbody>
</table>
</div>
...
<td>
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="noborders">
<tr>
<td>
<table border="0" cellspacing="0" cellpadding="0" width="100%">
<tr>
<td class="tdcontent">
<ul id="hy_priceask"><? if(is_array(json_decode($this->add['hy_price'],1))) { foreach(json_decode($this->add['hy_price'],1) as $key=>$value) { ?><li><input type="text" name="hy_price[<?=key?>][price]" value="<?=$value['price']?>" /> 元/件&nbsp;&nbsp;&nbsp; 起购：
<input type="text" name="hy_price[<?=key?>][minimum]" value="<?=$value['minimum']?>" />件
<span onclick="$(this).parent().remove();">删除</span></li><? } } ?></ul>
</td>
...
<tr>
<td class="tdcontent">
<ul id="tg_priceask"><? if(is_array(json_decode($this->add['tg_price'],1))) { foreach(json_decode($this->add['tg_price'],1) as $key=>$value) { ?><li><input type="text" name="tg_price[<?=key?>][price]" value="<?=$value['price']?>" /> 元/件&nbsp;&nbsp;&nbsp; 起购：
<input type="text" name="tg_price[<?=key?>][minimum]" value="<?=$value['minimum']?>" />件
<span onclick="$(this).parent().remove();">删除</span></li><? } } ?></ul>
</td>
...
<div class="headbar clearfix">
<ul class="tab" name="menu1">
<li<? if($_GET[ 'checked']=='' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(adminpre()); ?>">全部记录</a>
</li>
<li<? if($_GET[ 'checked']=='3' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(" ?mod=admin&act=shop&get=order&checked=9 "); ?>" hidefocus="true">商家确认中</a>
</li>
<li<? if($_GET[ 'checked']=='3' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(" ?mod=admin&act=shop&get=order&checked=10 "); ?>" hidefocus="true">平台确认中</a>
</li>
<li<? if($_GET[ 'checked']=='1' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(" ?mod=admin&act=shop&get=order&checked=1 "); ?>" hidefocus="true">待发货</a>
</li>
<li<? if($_GET[ 'checked']=='5' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(" ?mod=admin&act=shop&get=order&checked=5 "); ?>" hidefocus="true">已发货</a>
</li>
...
<input type="text" name="orderid" id="orderid" value="<?=$_GET['orderid']?>" class='skey w120' /> 时间段：<?=config::form('time',$this->t['str'],'datas');?>
...
<p>已上传凭证图片</p>
<a href="<?=$value['agent_img']?>" target="_blank"><img src="<?=$value['agent_img']?>" style="width:50px;" /></a><b style="color:#F30;"></b></td>
<td width="10%" id="check_user_<?=$value['id']?>">
<? if($value['nosend']) { ?>
库存不足，暂不可发货！
<? } else { ?><?=order_check_admin($value); } ?>
<p>
<? if($value['checked']=='5') { ?>
