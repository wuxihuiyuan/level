...
<!--我的订货-->
<? if($_GET['type']=='list') { if(!$this->member['is_admin'] ) { if($_GET['id']) { ?>
<div class="xm-box uc-box">
<div class="hd">
<h3 class="title">订货单号：<?=$this->order['id']?></h3>
</div><? $shop = $this->getgoods(1,"goods_id='".$this->order['goods_id']."'"); ?><div class="bd">
...
<td class="order-delivery-table" style="width:40%"><? $shop = $this->getgoods(1,"goods_id='".$value['goods_id']."'"); ?><div class="order-goods-info" style="padding-top:0;padding-bottom:0;"><?=$shop['goods_name']?></div>
...
<? if($value['status'] ==1) { ?>
<span price="<?=$this->order['price']?>" style="display: block;position: relative;top: 5px;">等待确认</span> 
</br><a href="<?=Purl(" ?mod=member&act=store&type=list&id=".$value['id']); ?>" style="padding: 2px 9px;width: 48px;margin: 0 auto;display: block;border: 1px solid #11bd11;color: #11bd11;border-radius: 3px;position: relative;top: -8px;">订单详情</a>
<? } elseif($value['status'] ==2) { ?>
<span price="<?=$this->order['price']?>" style="display: block;position: relative;top: 5px;">已完成</span> 
</br><a href="<?=Purl(" ?mod=member&act=store&type=list&id=".$value['id']); ?>" style="padding: 2px 9px;width: 48px;margin: 0 auto;display: block;border: 1px solid #11bd11;color: #11bd11;border-radius: 3px;position: relative;top: -6px;">订单详情</a>
<? } else { ?>
<a href="<?=Purl(" ?mod=member&act=store&type=list&id=".$value['id']); ?>" style="background: #4A8DFC;color: #fff;padding: 3px 10px;border-radius: 3px;display: block;margin: 0 auto;width: 48px;">确认支付</a>
<? } ?>
</td>
</tr>
</tbody><? } } ?></table>
...
<!--商品订货-->
<? if($_GET['type']=='detail') { if(!$this->member['is_admin']) { ?>
...
<td width="12%" style="line-height:24px;padding:10px 0;">
<? if($value['status'] ==1) { ?>
<span class="ordtin" style="display: block;">已付款</span><button id="confirmstore" data-value="<?=$value['id']?>" class="ent" style="background: #4A8DFC;color: #fff;padding: 5px 10px;border-radius: 3px;display: block;margin: 0 auto;width: 68px;cursor: pointer;">确认收款</button>
<? } elseif($value['status'] ==2) { ?>
