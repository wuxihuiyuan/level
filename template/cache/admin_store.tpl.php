...
<input type="text" name="id" id="orderid" value="<?=$_GET['id']?>" class='skey w120' /> 时间段：<?=config::form('time',$this->t['str'],'datas');?>
...
<tbody><? $shop = $this->getgoods(1,"goods_id='9'"); ?><tr>
...
<a href="<?=Purl(" ?mod=admin&act=store&get=store&checked=1 "); ?>" hidefocus="true">已付款</a>
</li>
<li<? if($_GET[ 'checked']=='2' ) { ?> class="selected" <? } ?>>
<a href="<?=Purl(" ?mod=admin&act=store&get=store&checked=2 "); ?>" hidefocus="true">已完成</a>
</li>
<div class="tabright">
<form method="get" action="">
<input type="hidden" name="mod" id="mod" value="<?=$_GET['mod']?>" />
...
<input type="text" name="orderid" id="orderid" value="<?=$_GET['id']?>" class='skey w120' /> 时间段：<?=config::form('time',$this->t['str'],'datas');?>
<input type="submit" id="button" value="立即搜索" class='button'>
</form>
</div>
<div style="clear:both;"></div>
