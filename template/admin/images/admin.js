$(function(){
		$("#all").click(function() {
			if(this.checked) {
				$(".purviews :checkbox").prop("checked", true);
			} else {
				$(".purviews :checkbox").prop("checked", false);
			};
		});
		$(".to_left").click(function(){
			$("#drop_down").stop().slideToggle();
			var text1 = $(this).text();
			if(text1 == "+"){
				 $(this).text("-") ;
			}else{
...
function checkgroupid(){
  ...
  if(groupid==''){
    addClass("catidtip","tipsno","对不起，请选择会员级别");
  }else{
    addClass("catidtip","tipsyes","&nbsp;");
  }
}
function checkcatid(){
  var groupid = $("#catid").val();
  if(groupid==''){
...
$(function(){
 ...
 $(".jforderback").click(function(){
  ...
  $(".jfordersend").click(function(){
  var id = $(this).attr('orderid');
  var _this = $(this);
    showhandle({
      html:'<table class="lotab"><tr><th><label>物流：</label></th><td><input id="express" class="myinput repass" type="text"></td></tr><tr><th><label>快递单号：</label></th><td><input id="expressnumber" class="myinput repass" type="text"></td></tr><tr><th><label>管理密码：</label></th><td><input id="repass" class="myinput repass" type="password"></td></tr></table>',
      width:600,
      height:250,
      id:'ordersend',
      title:'确认发货'
    ...
    $("#controlLoad").show();
    removetip("ordersend");
       $.getJSON(listTable.url+'&repass='+$("#repass").val()+'&do=ordersend&id='+id+'&express='+encodeURIComponent($("#express").val())+'&expressnumber='+encodeURIComponent($("#expressnumber").val())+'&repass='+encodeURIComponent($("#repass").val()),function(res) {
    $("#controlLoad").hide(); 
         if(res.error == 0){
 ...
 $('#confirmstore').click(function(event) {
    ...
    showhandle({
      ...
      $("#controlLoad").show();
      removetip("yeshave");
      $.getJSON(get_url('act=confirmstore&type=admin&repass='+$("#repass").val()+'&id='+id),function(res){
        $("#controlLoad").hide();                                    
        if(res.error=='0'){
   ...
   $('.orderok').click(function(event) {
      ...
      showhandle({
        ...
        $("#controlLoad").show();
        removetip("yeshave");
        $.getJSON(get_url('act=confirmorder&type=admin&repass='+$("#repass").val()+'&id='+id),function(res){
          $("#controlLoad").hide();                                    
          if(res.error=='0'){
            hidebox('yeshave',true);
             location.reload();
