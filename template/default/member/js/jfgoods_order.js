function backmoney(id){
  showhandle({
	...
	$("#controlLoad").show();
	removetip("backmoney");
    $.getJSON(get_url('act=jfbackmoney&repass='+$("#repass").val()+'&id='+id+'&message='+encodeURIComponent($("#message").val())),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
...
function yeshave(id){
  showhandle({
	...
	$("#controlLoad").show();
	removetip("yeshave");
    $.getJSON(get_url('act=jfyeshave&repass='+$("#repass").val()+'&id='+id),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
...
function payorder(id){
  showhandle({
    ...
    height:140,
    id:'payorder',
    title:'确定要使用'+$("#check_user_"+id).attr('price')+"积分吗？"
  },function(){
	$("#controlLoad").show();
	removetip("payorder");
    $.getJSON(get_url('act=payjforder&repass='+$("#repass").val()+'&id='+id),function(res){
	  $("#controlLoad").hide();																		 
	  if(res.error=='0'){
