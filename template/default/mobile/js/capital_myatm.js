  ...
  $("#opcardbutton").click(function(){
	...
	if(post){
      showhandle({
        ...
        $.getJSON(get_path_url("?mod=mobile&act=capital&type=myatm&bankid="+atmbank+"&money="+howmoney+"&repass="+$("#repass").val()),function(res){
	      $("#controlLoad").hide();																		 
	      if(res.error=='0'){
...
function addbank(){
  sinbox('添加银行',get_path_url("?mod=mobile&act=bankcard&do=add"),480,350);
}
function editbank(id){
  sinbox('修改银行',get_path_url("?mod=mobile&act=bankcard&do=edit&id="+id),480,350);
}
function delbank(id){
  showAsk({Msg:"确定要删除该银行卡吗"},function(){
    $.ajax({url:get_path_url("?mod=mobile&act=bankcard&do=del&id="+id),
      type:'GET',
      dataType:"json",
...
function pagereload(){
  showLoading();
  $.getJSON(get_path_url("?mod=mobile&act=bankcard&do=list"),function(res){
	if(res==null){
	  $("#banklist").html('');	
	  $("#hasSetBanks").show();	
	  $("#banks").show();
