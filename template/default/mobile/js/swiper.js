...
var Swiper=function(f,b){
	...
	function v(c){
		if(a.isTouched&&!b.onlyExternal&&!(u&&"mousemove"==c.type)){
			...
			if(!w)if(c.assignedToSwiper)a.isTouched=!1;
				...
				if(b.resistance){
					if(0<a.positions.current&&(!b.freeMode||b.freeModeFluid))b.loop?(c=1,0<a.positions.current&&(a.positions.current=0)):c=(2*l-a.positions.current)/l/2,a.positions.current=0.5>c?l/2:a.positions.current*c;
					if(a.positions.current<-g()&&(!b.freeMode||b.freeModeFluid))b.loop?(c=1,d=a.positions.current,e=-g()-l):(d=(a.touches.current-a.touches.start)*b.ratio+(g()+a.positions.start),c=(l+d)/l,d=a.positions.current-d*(1-c)/2,e=-g()-l/2),a.positions.current=d<e||0>=c?e:d
				}
				if(b.followFinger){
	...
	if(f.nodeType||document.querySelectorAll&&0!=document.querySelectorAll(f).length){
		$(f).height(parseInt($(".swiper-slide").width())*0.55);					   
		var a=this;
		a.touches={};
		a.positions={
			current:0
		...
		var n,h,p,k,y,x,w,l;
		for(m=a.container.childNodes.length-1;0<=m;m--)if(a.container.childNodes[m].className)for(var q=a.container.childNodes[m].className.split(" "),F=0;
		F<q.length;
		F++)q[F]===b.wrapperClass&&(n=a.container.childNodes[m]);
		a.wrapper=n;
		h="horizontal"==b.mode;
		a.touchEvents={
			touchStart:a.support.touch||!b.simulateTouch?"touchstart":a.ie10?"MSPointerDown":"mousedown",touchMove:a.support.touch||!b.simulateTouch?"touchmove":a.ie10?"MSPointerMove":"mousemove",touchEnd:a.support.touch||!b.simulateTouch?"touchend":a.ie10?"MSPointerUp":"mouseup"
		};
		...
		a._calcSlides=function(){
			...
			a.slides=[];
			for(var d=0;d<a.wrapper.childNodes.length;d++)if(a.wrapper.childNodes[d].className)for(var e=a.wrapper.childNodes[d].className.split(" "),f=0;
			f<e.length;
			f++)e[f]===b.slideClass&&a.slides.push(a.wrapper.childNodes[d]);
		...
		a.updatePagination=function(){
			if(!(2>a.slides.length)){
				...
				if(c){
					...
					for(var c=document.querySelectorAll(b.pagination+" ."+b.paginationClass).length,e=b.loop?a.realIndex-b.slidesPerSlide:a.realIndex,f=e+(b.slidesPerSlide-1),d=e;
					d<=f;
					d++){
						var g=d;
						g>=c&&(g-=c);
		...
		if(b.mousewheelControl){
			...
			catch(J){}a._wheelEvent||(a._wheelEvent="DOMMouseScroll");
			var H=function(c){
		...
		a.swipeNext=function(c){
			...
			c=Math.abs(c);
			if(d!=y&&(!(c>=g())||b.loop))return d>g()&&!b.loop&&(d=g()),b.loop&&d>=g()+l&&(d=g()+l),h?a.setTransform(-d,0,0):a.setTransform(0,-d,0),a.setTransition(b.speed),a.updateActiveSlide(-d),E(),!0
		};
		...
		a.swipeTo=function(c,d,e){
			...
			if(!(c>k-1)&&(!(0>c)||b.loop))return e=!1===e?!1:e||!0,d=0===d?d:d||b.speed,b.loop&&(c+=b.slidesPerSlide),c>k-b.slidesPerSlide&&(c=k-b.slidesPerSlide),c=-c*p,I&&(b.loop&&0<b.initialSlide&&b.initialSlide<k)&&(c-=b.initialSlide*p,I=!1),h?a.setTransform(c,0,0):a.setTransform(0,c,0),a.setTransition(d),a.updateActiveSlide(c),e&&E(),!0
		};
...
Swiper.prototype={
	plugins:{},transitionEnd:function(f,b){
