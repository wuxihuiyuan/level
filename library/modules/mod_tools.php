...
class mod_tools extends module_class{
	...
	function payjforder(){
	  ...
	  $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}' and uid='{$this->member['uid']}' and checked='0'");
	  if(is_array($order)){
		if(!$_GET['repass']){
		  $arr['error'] = "请输入支付密码取消订单";
		}else{
	...
	function confirmstore(){
    	...
    	if($_GET['type']!= 'admin' && !$this->member['is_admin']){
    		if($this->member['store']<$order['store']){
    			$arr['error'] = '当前库存不足！';
    			echo json($arr);
    			exit;
    		}
		...
		//开通会员
		if(!$userdata['status']){
			$this->status($userdata['uid'],'1',$order);
		}
		$this->up_store($userdata,"+",$order); 

		//确定用户
		//确定log
		if($_GET['type']== 'admin'){
			$this->add_storelog($this->manager,'confirm',"admin",$order);			
		}else{
			$this->add_storelog($this->member,'confirm',"user",$order);
		}
		//上级库存
		if($_GET['type']!= 'admin' && !$this->member['is_admin']){
			$this->up_store($this->member,"-",$order); 
		}
	  $arr['error'] = '0';
	  echo json($arr);
	  exit;
	}
	//type pay 下级订货单 confirm 上级或后台确认订货
	//confirm_type user 上级确认 admin 后台确认
	function add_storelog($user,$type,$confirm_type="user",$order){
		$log['type'] = $type;
		$log['confirm_type'] = $confirm_type;	
		$log['uid'] = $user['uid'];
		$log['oid'] = $order['id'];
	...
	function up_store($user,$action="+",$order){
		if($action == "+"){
			$update['store'] = intval($user['store']) + $order['store'];
		}
		if($action == "-"){
			$update['store'] = intval($user['store']) - $order['store'];
		}
		$result = $this->mysql->update("{$this->pre}user",$update,"uid='{$user[uid]}'"); 
		return "0";
	}
	function up_point($user,$order){
		$group = $this->mysql->select_one("select * from {$this->pre}usergroup where groupid='{$user['groupid']}'");
		$treeids = $user['treeids'];
		if($group['sort']>1){
			if($treeids){
				$treeids = $user['treeids'].','.$user['uid'];
			}else{
				$treeids = $user['uid'];
			}
		}
		if($treeids){
			$users = $this->mysql->getarr("select * from {$this->pre}user where uid in({$treeids})");
			foreach ($users as $key => $value) {
				$update = array();
				$update['good_point'] = $value['good_point'] + $order['point'];
				$this->mysql->update("{$this->pre}user",$update,"uid='{$value[uid]}'"); 
			}
		}
		return "0";
	}
	function confirmorder(){
		...
		if($_GET['type']== 'user'){
    		...
    		$update['send_type'] = 'user';
		    $order = $this->mysql->select_one("select * from {$this->pre}order where id='{$_GET['id']}' and checked='9'");
		    $order['send_type'] = 'user';
		}
  	  	$arr['error'] = '0';
  	  	...
  	  	if(!$user['status']){
  	  		$this->status($user['uid'],'1',$order);
  	  	}
  	  	if($_GET['type']== 'admin'){
	  	  	//积分处理(一级发货或城市发货都不考虑积分)
	  	  	$this->up_point($user,$order);	
  	  	}
  	  	//奖励更新
		$this->bonus($user,$order);
  	  	//更新订单
  	  	$update['checked'] = 1;
  	  	$this->mysql->update("{$this->pre}order",$update,"id='$order[id]'"); 
  	  	echo json($arr);
	...
	function jfbackmoney(){
	  $arr['error'] = '0';
	  $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}' and uid='{$this->member['uid']}'");
	  if(is_array($order)){
		if(!$_GET['repass']){
		  ...
		  if($this->user->password($_GET['repass'],$this->member['salt'])==$this->member['repass']){
            ...
            $error = $this->turnjforder($order,3);
	...
	function jfyeshave(){
	  $arr['error'] = '0';
	  $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}' and checked='5' and uid='{$this->member['uid']}'");
	  if(is_array($order)){
		if(!$_GET['repass']){
		  ...
		  if($this->user->password($_GET['repass'],$this->member['salt'])==$this->member['repass']){
            ...
            $error = $this->turnjforder($order,2);
			if($error) $arr['error'] = $error;
		  }else{
		    $arr['error'] = "支付密码不正确";
		  }
