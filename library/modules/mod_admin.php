...
class mod_admin extends module_class{
	...
	function shop(){  
	  if($_GET['get']=='goods'){
		if($_GET['re']=='list'){
		    ...
		    $group = $this->mysql->getarr("select groupid,groupname from {$this->pre}usergroup  order by sort asc");
	  ...
	  if($_GET['get']=='order'){	
	    ...
	    if($_GET['re']=='list'){
	      ...
	      foreach ($order as $key => $value) {
	      	$user = $this->mysql->select_one("select uid,is_admin,store from {$this->pre}user where uid={$value['checkid']}");
	      	if(!$user['is_admin'] && $value['checked']==1 && $user['store']<$value['num']){
	      		$order[$key]['nosend'] = 1;
	      	}
	      }
	      $this->order = $order;
	...
	function jfshop(){  
		$_GET['re'] = $_GET['re'] ? $_GET['re'] : "list";

		if($_GET['get']=='category'){
				    if($_GET['re']=='list'){
			          $this->category = $this->mysql->getarr("select * from {$this->pre}category order by sort asc");
				    }	    
				    if($_GET['re']=='add'||$_GET['re']=='edit'){
			          $this->formtitle = $_GET['re']=='add' ? '添加分类' : "编辑分类";
					  $id = $_GET['id'];
			          ...
			          if(submit()){
					    ...
					    if($id){
						  $message = '编辑会员级别成功';
			              $this->mysql->update("{$this->pre}category",$arr,"id='$groupid'"); 
					    }else{
						  $message = '添加会员级别成功';
			              $this->mysql->insert("{$this->pre}category",$arr); 
					    }
						$json['error'] = "0";
						$json['message'] = $message;
						$json['url'] = Purl('?mod=admin&act=jfshop&get=category');
						echo json($json);
					    exit;
				      }
				 	  if($_GET['re']=='edit'){
			            $this->add = $this->mysql->select_one("select * from {$this->pre}category where id='$id'");
				        if(!is_array($this->add)) $this->message('go_back','该会员级别不存在');
					  }
				    } 		
		}
		...
		if($_GET['get']=='goods'){
			...
			if($_GET['re']=='list'){
				...
				$keyword = $_GET['keyword'];
				if($keyword) $where.=" and goods_name like '%{$keyword}%'";
			    $this->pagetotal = $this->mysql->counts("select * from {$this->pre}jfgoods $where");
				$this->pageclass($this->pagetotal);
			    $query = $this->mysql->query("select * from {$this->pre}jfgoods $where ".getsort("goods_id_desc")." {$this->page->limit}");
			    while($rs=$this->mysql->assoc($query)){
			    	$catone = $this->mysql->select_one("select * from {$this->pre}category where id='{$rs['catid']}'");
			    	$rs['catname'] = $catone['name'];
				    $rs['editurl'] = rewrite::request("?mod={$this->module}&act={$this->action}&get=goods&re=edit&id=".$rs['goods_id']);
				    $this->goodslist[]=$rs;
			    }
			}		
		    if($_GET['re']=='add'||$_GET['re']=='edit'){
			  	$id = $_GET['id'];
			  	$this->usercatlist = $this->getselect('category','id','name','','order by sort asc',"[<><请选择商品分类>]");
			  	$this->shoptitle = $_GET['re']=='add' ? "添加商品" : "编辑商品";
			  	$this->add = $this->mysql->select_one("select * from {$this->pre}jfgoods where goods_id='$id'");
	          	if(submit()){
					$json['error'] = "0";
			    	if($this->manager_class->password($_GET['repass'],$this->manager['salt'])!=$this->manager['password']){
		              	$json['error'] = "管理密码不正确";
		            ...
		            $arr['point_rate'] = $_POST['point_rate'];		
					$arr['margin'] = $_POST['margin'];
					$arr['stock'] = $_POST['stock'];
					$arr['catid'] = $_POST['catid'];
				    if($id){
					  $json['message'] = '编辑商品成功';
		              $this->mysql->update("{$this->pre}jfgoods",$arr,"goods_id='$id'"); 
				    }else{
					  $arr['addtime'] = time();
				 	  $json['message'] = '添加商品成功';
		              $this->mysql->insert("{$this->pre}jfgoods",$arr); 
				    }
					$json['url'] = Purl("?mod=admin&act=jfshop&get=goods");
					echo json($json);
					exit;
		    	}
			    if($_GET['re']=='edit'){
		    ...
		    if($_GET['re']=='ajax'){
			    if($this->manager_class->password($_GET['repass'],$this->manager['salt'])!=$this->manager['password']){
			  	...
			  	if($_GET['do']=='ischeck'){
					$arr['ischeck'] = $_GET['val'];
					$this->mysql->update("{$this->pre}jfgoods",$arr,"goods_id='{$_GET['id']}'");
					$arr['error'] = 0;
			        echo json($arr);
			  	}elseif($_GET['do']=='remove'){
					$this->mysql->delete("{$this->pre}jfgoods","goods_id='{$_GET['id']}'");
				    $arr['error'] = 0;
			        echo json($arr);
			  	}
			    exit;
	  	...
	  	if($_GET['get']=='order'){
		    ...
		    if($_GET['re']=='list'){
		      ...
		      $this->pageclass($this->pagetotal);
		      $this->order = $this->mysql->getarr("select * from {$this->pre}jforder {$where} order by id desc {$this->page->limit}");
			  $this->express = _formatval(config::get("express"),'[<><请选择快递>]');
		    }
		    if($_GET['re']=='ajax'){
			  if($this->manager_class->password($_GET['repass'],$this->manager['salt'])!=$this->manager['password']){
			  ...
			  if($_GET['do']=='backmoney'){
				 $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}'");
				 $arr['error'] = $this->turnjforder($order,'4');
		         echo json($arr);
		      }
			  if($_GET['do']=='ordersend'){
				 $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}'");
				 ...
				 $arr['error'] = $this->turnjforder($order,'5');
		         echo json($arr);
		      }
			  if($_GET['do']=='nobackmoney'){
				 $order = $this->mysql->select_one("select * from {$this->pre}jforder where id='{$_GET['id']}'");
				 $order['messagea'] = $_GET['message'];
				 if($order['messagea']){
				   $arr['error'] = $this->turnjforder($order,'1');
				 }else{
				   $arr['error'] = '请输入拒绝原因';	 
				 }
				 echo json($arr);
